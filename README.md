...
설치 후 새 Claude Code 세션에서 `/skill-create` 또는 `/에신-llm-dependent-agent-create`를 호출하면 됩니다. Windows·macOS·Linux 모두 동일.
...
3. 압축 풀고 두 폴더(`skill-create`, `에신-llm-dependent-agent-create`)를 `~/.claude/skills/` 안으로 복사
...
ls ~/.claude/skills/ | grep -E "skill-create|에신-llm-dependent-agent-create"
```
