...
**에이전트 만드는 스킬만 받기** (`/에신-llm-dependent-agent-create`)
> **"`SUNWOONGKYU/claude-meta-skills` 에서 `에신-llm-dependent-agent-create`만 받아서 내 `~/.claude/skills/`에 설치해 줘."**

**스킬 만드는 스킬만 받기** (`/skill-create-코어5`)
> **"`SUNWOONGKYU/claude-meta-skills` 에서 `skill-create-코어5`만 받아서 내 `~/.claude/skills/`에 설치해 줘."**

### 공통 사항

설치 후 새 Claude Code 세션에서 `/skill-create-코어5` 또는 `/에신-llm-dependent-agent-create`를 호출하면 됩니다. Windows·macOS·Linux 모두 동일.
...
3. 압축 풀고 두 폴더(`skill-create-코어5`, `에신-llm-dependent-agent-create`)를 `~/.claude/skills/` 안으로 복사
...
# /skill-create
New-Item -ItemType Directory -Force "$skillsDir\skill-create-코어5" | Out-Null
Invoke-WebRequest "https://raw.githubusercontent.com/SUNWOONGKYU/claude-meta-skills/main/skill-create-코어5/SKILL.md" `
    -OutFile "$skillsDir\skill-create-코어5\SKILL.md"

# /에신-llm-dependent-agent-create
New-Item -ItemType Directory -Force "$skillsDir\에신-llm-dependent-agent-create" | Out-Null
Invoke-WebRequest "https://raw.githubusercontent.com/SUNWOONGKYU/claude-meta-skills/main/에신-llm-dependent-agent-create/SKILL.md" `
    -OutFile "$skillsDir\에신-llm-dependent-agent-create\SKILL.md"
```

**macOS·Linux (curl)**
```bash
mkdir -p ~/.claude/skills/skill-create-코어5 ~/.claude/skills/에신-llm-dependent-agent-create
...
ls ~/.claude/skills/ | grep -E "skill-create-코어5|에신-llm-dependent-agent-create"
```

Claude Code 새 세션에서 다음 명령이 자동 인식되면 정상 설치:
- `/skill-create-코어5 [만들 스킬 설명]`
- `/에신-llm-dependent-agent-create [만들 에이전트 설명]`
